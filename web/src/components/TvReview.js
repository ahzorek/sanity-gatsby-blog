  ...
  async function fetchData() {
    ...
    const {data} = response
      setInfo({
        ...
        homepage: data.homepage ? data.homepage : false,
        isLoaded: true
      })
  }
  useEffect(() => { fetchData() }, [showInfo.isLoaded] )

  if(showInfo.isLoaded === false) { return <div>Loading...</div> } 
  else {
    const { title, poster, release, desc, score, genre, eps, seasons, network, homepage } = showInfo
    return (
