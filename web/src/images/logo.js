...
`
const Logo = ({ color = 'rgb(23,23,23)', hideText }) => {
  return (
    <StaticQuery
      query={SiteNameQuery}
      render={data => {
        ...
        return (
          <LogoWrapper title={title}>
            <svg fill={color} style={{width: 30, height: 42}} viewBox="0 0 30 42">
            ...
            {!hideText && <HibernativosTitle fontcolor={color}>{title}</HibernativosTitle>}
          </LogoWrapper>
        )
      }}
    />
